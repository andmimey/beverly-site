...
- **One typeface, one size**: Archivo (grotesque, self-hosted variable) at 14px everywhere; exactly two weights — regular for prose, nav, and tracked-caps labels; bold for titles and kickers plus uppercase at a uniform 0.06em tracking. The wordmark is the only larger element: "BACH ARCHITECTURE" set live at 22px in Roboto Condensed 500 — a stand-in for the logo's Helvetica Condensed Medium-class face (the site's hosted Germano Regular is too light; licensed Helvetica Now Text Condensed can be swapped in via `fonts/`).
